...
func TestWorkerGracefulShutdownMidBatchNoLossNoDuplicates(t *testing.T) {
	...
	worker1, err := NewWorker(repository, processor1, handler, nil, nil, testLogger(), WorkerConfig{
		WorkerID:        "worker-a",
	...
	worker2, err := NewWorker(repository, processor2, handler, nil, nil, testLogger(), WorkerConfig{
		WorkerID:        "worker-b",
		Queues:          []domain.Queue{"default"},
		Concurrency:     10,
		BatchSize:       10,
...
func TestWorkerRecoversPanicAndKeepsRunning(t *testing.T) {
	...
	worker, err := NewWorker(repository, processor, handler, nil, nil, testLogger(), WorkerConfig{
